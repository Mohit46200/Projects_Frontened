...
const Header = () => {
  const { cart, login, userLoginData } = useContext(Globalcontext)
  ...
  return (
    <motion.header
      ...
      className={`sticky top-0 z-50 backdrop-blur-xl bg-white/80 border-b transition-colors duration-300 ${
        scrolled ? "border-[#efe3dc] shadow-sm" : "border-transparent"
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <motion.div
          animate={{ height: scrolled ? 76 : 96 }}
        ...
        >
                ...
                return (
                  <Link
                    ...
                    className={`relative py-2 text-sm uppercase tracking-[0.2em] transition ${
                      active ? "text-[#6b1d3a]" : "text-gray-600 hover:text-[#6b1d3a]"
                    }`}
                  >
                    {item.label}
                    {active && (
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            >
              ...
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] text-gray-400">Account</p>
                <p className="text-sm text-gray-700 font-medium">{login ? "Profile" : "Login"}</p>
              </div>
            </Link>
            ...
            >
              <motion.div className="relative" whileHover={{ rotate: [0, -8, 8, 0] }} transition={{ duration: 0.5 }}>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <div className="hidden sm:block">
                <p className="text-[10px] uppercase tracking-[0.25em] text-white/60">Shopping</p>
        ...
        {menuOpen && (
          ...
          >
            <div className="px-6 py-6 flex flex-col gap-1">
                ...
                >
                  <Link
                    ...
                    className={`block py-3 text-base uppercase tracking-[0.2em] border-b border-[#f4ece8] ${
                      location.pathname === item.to ? "text-[#6b1d3a]" : "text-gray-600"
                    }`}
                  >
                    {item.label}
                  </Link>
              ...
              >
                <Link to="/login" className="block py-3 text-base uppercase tracking-[0.2em] text-gray-600">
                  {login ? "Profile" : "Login"}
                </Link>
              </motion.div>
            </div>
